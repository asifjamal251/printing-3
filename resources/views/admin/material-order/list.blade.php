...
<ul id="customContextMenu" class="p-0 dropdown-menu dropdown-menu dropdownmenu-success" style="min-width:200pxdisplay:none; position:absolute; z-index: 10000;">
    @can('add_material_order')
        <li>
            <a class="dropdown-item " data-url="{{ route('admin.'.request()->segment(2).'.create') }}" href="javascript:void(0)" id="contextCreate">
                <i class="align-top fs-16 bx bx-plus me-2"></i> Add New</a>
        </li>
    @endcan
    @can('edit_material_order')
        <li>
            <a class="dropdown-item " id="contextEdit" href="javascript:void(0)"><i class="align-top ri-pencil-fill me-2"></i> Edit</a>
        </li>
    @endcan


    @can('read_material_order')
        <li><a class="dropdown-item " href="#" id="contextView"><i class="align-top ri-eye-fill me-2"></i> View</a></li>
    @endcan


    @can('add_material_inward')
        <li>
            <a class="dropdown-item " data-url="{{ route('admin.'.request()->segment(2).'.create') }}" href="javascript:void(0)" id="contextInward">
                <i class="align-top fs-16 ri-login-circle-fill me-2"></i> Add Inward</a>
        </li>
    @endcan
    ...
    <div class="dropdown-divider m-0"></div>
        <li><a class="dropdown-item " href="javascript:void(0)" id="contextDelete"><i class="align-top ri-delete-bin-fill me-2"></i> Delete</a></li>
    @endcan
</ul>
    ...
    var table2 = $('#datatable').DataTable({
        ...
        $('body').on('change', '.onChange', function(){
            ...
            $('#customContextMenu').on('click', 'a', function(e) {
                ...
                if (rowData && rowData.status_id == 3) {
                    $('#contextEdit').closest('li').hide();
                } 

                if (rowData && rowData.status_id == 1){
                    $('#contextEdit').closest('li').show();
                }
                ...
                if (rowData) {
                    ...
                    if (this.id === 'contextInward') {
                        var inwardUrl = '{{route('admin.material-inward.create')}}?order_id='+rowData.id;
                        window.location.href = inwardUrl;
                    }
