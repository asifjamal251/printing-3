...
<ul id="customContextMenu" class="p-0 dropdown-menu dropdown-menu dropdownmenu-secondary" style="min-width:200pxdisplay:none; position:absolute; z-index: 10000;">
    ...
    <li>
        <a class="dropdown-item editData" model-size="modal-xl" data-title="Add Box Details" href="javascript:void(0)" id="contextAddDetails" bg-color="#f3f3f9"><i class="bx bx-plus me-2"></i> Add Details</a>
    </li>

    <li>
        <a class="dropdown-item" href="javascript:void(0)" id="contextSendToWarehouse" bg-color="#f3f3f9"><i class="bx bx-plus me-2"></i> Send To Warehouse</a>
    </li>


    <li>
        <a class="dropdown-item" href="javascript:void(0)" id="contextMakeComplete" bg-color="#f3f3f9"><i class="bx bx-plus me-2"></i> Make Complete</a>
    </li>
            ...
            <div class="card-body border border-dashed border-end-0 border-start-0">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 col-sm-12">
                        <div class=" form-group{{ $errors->has('filter_mfg_by') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-3 col-sm-12">
                        <div class=" form-group{{ $errors->has('filter_mkdt_by') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-3 col-sm-12">
                        <div class="search-box">
                            <div class=" form-group{{ $errors->has('filter_job_no') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-3 col-sm-12">
                        <div class="m-0 form-group{{ $errors->has('filter_operator') ? ' has-error' : '' }}">
                            {{ html()->select('filter_operator', App\Models\Operator::orderBy('name', 'asc')->where('module_id', 9)->pluck('name', 'id'))->id('filterOperator')->class('form-control onChange js-choice')->placeholder('Choose Operator') }}
                    ...
                    <div class="col-md-3 col-sm-12">
                        <div class="m-0 form-group{{ $errors->has("filter_dye") ? ' has-error' : '' }}">
                            {{ html()->select("filter_dye", [])->id('filterDye')->class('form-control dye onChange')->placeholder('Die') }}
                            <small class="text-danger">{{ $errors->first("filter_dye") }}</small>
                        </div>
                    </div>
                    ...
                    <div class="col-md-3 col-sm-12">
                        <div class="m-0 form-group{{ $errors->has('filterStatus') ? ' has-error' : '' }}">
                            {{ html()->select('filterStatus', App\Models\Status::whereIn('id', [1,2,3])->pluck('name', 'id'))->id('filterStatus')->class('form-control js-choice onChange')->placeholder('Status') }}
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#customContextMenu').on('click', 'a', function(e) {
            ...
            if (rowData) {
                ...
                if (this.id === 'contextSendToWarehouse') {
                    var url = window.location.href + '/send/warehouse/' + rowData.id;
                    updateData(url, { id: rowData.id });
                }

                if (this.id === 'contextMakeComplete') {
                    var url = window.location.href + '/make/complete/' + rowData.id;
                    updateData(url, { id: rowData.id }, 'Complete Pating Item', 'You Can not backe reverted, please confirm');
                }
        ...
        function handleContextMenuRestrictions(rowData) {
            const statusId = rowData?.status_id;
            const restrictedStatuses = [3];
            const sendToWarehouse = [2];
            const addDetails = [2, 1];
            ...
            if (sendToWarehouse.includes(statusId)) {
                $('#contextSendToWarehouse')
                .removeClass('disabled')
                .css('pointer-events', '')
                .css('opacity', '');
