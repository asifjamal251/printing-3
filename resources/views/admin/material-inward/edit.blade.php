...
<div class="card border border-success mb-5">
    <div class="card-body">
        <div class="row">
            {{ html()->hidden('order_id', $order->material_order_id) }}
            ...
            <div class="col-md-3 col-sm-12">
                <div class="form-group{{ $errors->has('bill_no') ? ' has-error' : '' }}">
                    ...
                    {{ html()->text('bill_no', $order->bill_no)->class('form-control')->placeholder('Bill No.') }}
            ...
            <div class="col-md-3 col-sm-12">
                <div class="form-group{{ $errors->has('bill_date') ? ' has-error' : '' }}">
                    ...
                    {{ html()->text('bill_date', $order->bill_date->format('d F, Y'))->id('bill_date')->placeholder('Bill Date')->class('dateSelector form-control') }}
    ...
    <div id="kt_docs_repeater_advanced">
        <div data-repeater-list="kt_docs_repeater_advanced">
            @foreach(old('kt_docs_repeater_advanced', $order->items ?? [[]]) as $index => $item)
                @php
                    $product = App\Models\Product::with(['productType', 'category'])->find($item['product_id'] ?? null);
                @endphp
                <div data-repeater-item class="repeater-row row-{{$index}}">
                    {{ html()->hidden("kt_docs_repeater_advanced[$index][id]", old("kt_docs_repeater_advanced.$index.id", $item['id'] ?? '')) }}
                    {{ html()->hidden("kt_docs_repeater_advanced[$index][order_item_id]", old("kt_docs_repeater_advanced.$index.order_item_id", $item['material_order_item_id'] ?? '')) }}
                        ...
                        <div class="card-body">
                            <div class="product-row custom-row gap-3 stock-error d-flex justify-content-between flex-sm-wrape">
                                <div class="w-100 form-group{{$errors->has('kt_docs_repeater_advanced.'.$loop->index.'.product') ? ' has-error' : '' }}">
                                    ...
                                    <select name="kt_docs_repeater_advanced[{{ $index }}][product]" class="form-control form-select" data-kt-repeater="select2" data-placeholder="Select an option">
                                ...
                                {{ html()->hidden("kt_docs_repeater_advanced[$index][old_quantity]", old("kt_docs_repeater_advanced.$index.old_quantity", $item['quantity'] ?? '')) }}
                            ...
                            <div class="product-row custom-row gap-3 stock-error d-flex justify-content-between flex-sm-wrape">
                                <div class="w-75 form-group{{ $errors->has("kt_docs_repeater_advanced.$index.total_weight") ? ' has-error' : '' }}">
                                    {{ html()->label('Total WT') }}
                                ...
                                <div class="w-75 form-group{{ $errors->has("kt_docs_repeater_advanced.$index.rate") ? ' has-error' : '' }}">
                                    {{ html()->label('Rate') }}
                                    @can('rate_product')
                                ...
                                <div class="w-75 form-group{{ $errors->has("kt_docs_repeater_advanced.$index.gst") ? ' has-error' : '' }}">
                                    {{ html()->label('GST') }}
                                    <div class="input-group">
                                ...
                                <div class="w-75 form-group{{ $errors->has("kt_docs_repeater_advanced.$index.amount") ? ' has-error' : '' }}">
                                    {{ html()->label('Amount') }}
                                    <div class="input-group">
                                        <span class="input-group-text">₹</span>
                                        {{ html()->text("kt_docs_repeater_advanced[$index][amount]", old("kt_docs_repeater_advanced.$index.amount", $item['amount'] ?? ''))->class('form-control')->attribute('readonly')->placeholder('Amount') }}
                                    </div>
                                    <small class="text-danger">{{ $errors->first("kt_docs_repeater_advanced.$index.amount") }}</small>
                                </div>

                                <div class="form-group remove-item" style="width:44px;">
                                    <div class="text-end">
   ...
   $('#kt_docs_repeater_advanced').repeater({
    ...
    hide: function (deleteElement) {
        ...
        if (confirm('Are you sure to perform this action')){
            $.ajax({
               url: '{{ route('admin.material-inward.destroy.item', 0) }}'.replace('/0', '/' + deletedId),
                method: 'post',
