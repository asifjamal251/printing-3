...
class JobCardStageService
{
    public static function updateStageCounter(JobCardStage $stage, int $outCounter){
        ...
        if ($nextStage) {
            // set in_counter for next stage
            $nextStage->in_counter = $outCounter;
            $nextStage->save();

            // dynamically create model record
    ...
    public static function cancelStage(JobCardStage $stage){
        DB::transaction(function () use ($stage) {
            ...
            if ($nextStage) {
                ...
                if (class_exists($modelClass)) {
                    ...
                    if ($modelRecord) {
                        ...
                        // safe to delete if pending
                        $modelRecord->delete();
                    }
                }

                // 2. reset next stage counters and status
                $nextStage->update([
                    'in_counter'  => null,
                    'out_counter' => null,
                    'status_id'   => null,
            ...
            // 3. reset current stage (Printing etc.)
            $stage->update([
                'out_counter' => null,
                'status_id'   => 1,
            ]);
    ...
    //         // now dynamically create model record
    //         $modelClass = "App\\Models\\" . $nextStage->name;
    ...
    //     // update next stage’s in_counter
    //     $nextStage = JobCardStage::where('job_card_id', $stage->job_card_id)
    //         ->where('order', '>', $stage->order)
    //         ->orderBy('order', 'asc')
    //         ->first();
    ...
    //     if ($nextStage) {
    //         // next stage gets previous out_counter as input
    //         $nextStage->in_counter = $outCounter;
    //         $nextStage->save();
    //     }
    // }
