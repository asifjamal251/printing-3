...
if (!function_exists('tablePOItems')) {
    function tablePOItems($poId)
    {
        ...
        $accordionHtml .= '>';
        $accordionHtml .= '<span><strong>Item:</strong> '.$first->item_name.' </span> | <span><strong>Simmze:</strong> '.$first->item_size.' </span>| <span><strong>Quantity:</strong> '.$first?->quantity . '</span>';
        ...
        if ($hasMore) {
            ...
            foreach ($items->skip(1)->values() as $i => $item) {
                ...
                $accordionHtml .= '<div class="ps-1 accordion-row'.($isLast ? ' last-row' : '').'"><span><strong>Item:</strong> '.$item->item_name.' </span>| <span><strong>Size:</strong> '.$item->item_size.' </span>| <span><strong>Quantity:</strong> '.$item?->quantity . '</span></div>';
            }
            $accordionHtml .= '</div>';
        }
...
if (!function_exists('tableJobCardItems')) {
    function tableJobCardItems($jobCardId)
    {
        ...
        $accordionHtml .= '>';
        $accordionHtml .= '<span><strong>Item:</strong> '.$first?->itemProcessDetail?->item?->item_name.' </span>| <span><strong>Size:</strong> '.$first?->itemProcessDetail?->item?->item_size.' </span> | <span> <strong>GSM:</strong> '.$first?->itemProcessDetail?->gsm.' </span>| <span><strong>Colour:</strong> '.$first?->itemProcessDetail?->item?->colour . ' </span>| <span><strong>Quantity:</strong> '.$first?->itemProcessDetail?->quantity .'</span>';
        ...
        if ($hasMore) {
            ...
            foreach ($items->skip(1)->values() as $i => $job_card_item) {
                $isLast = $i === $lastIndex - 1;
                $accordionHtml .= '<div class="ps-1 accordion-row'.($isLast ? ' last-row' : '').'"><span><strong>Item:</strong> '.$job_card_item?->itemProcessDetail?->item?->item_name.' </span>| <span><strong>Size:</strong> '.$job_card_item?->itemProcessDetail?->item?->item_size.' </span>| <span> <strong>GSM:</strong> '.$job_card_item?->itemProcessDetail?->gsm.' </span>| <span><strong>Colour:</strong> '.$job_card_item?->itemProcessDetail?->color .' </span>|<span> <strong>Quantity:</strong> '.$job_card_item?->itemProcessDetail?->quantity . '</span></div>';
            }
            $accordionHtml .= '</div>';
        }
