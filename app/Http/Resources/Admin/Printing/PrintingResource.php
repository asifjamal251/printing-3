    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'coating' =>
                        // Main coating(s)
                        $this->jobCard->items
                            ->map(fn ($item) => $item->coatingType?->name)
                            ->filter()
                            ->unique()
                            ->implode(', ') . ' (' . $this->jobCard->printing .')'
                        .
                        // Line break only if other coating exists
                        (
                            $this->jobCard->items
                                ->map(fn ($item) => $item->otherCoatingType?->name)
                                ->filter()
                                ->unique()
                                ->isNotEmpty()
            ...
            'color' => $this->jobCard->items->pluck('colour')->filter()->unique()->implode(', '),
