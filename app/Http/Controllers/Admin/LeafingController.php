...
class LeafingController extends Controller
{
    ...
    public function updateOperator(Request $request){
        if($request->operator_id){
            ...
            $leafing->update(['operator_id' => $operator->id, 'admin_id' => $operator->admin_id]);
            JobCardStage::where('id', $leafing->job_card_stage_id)->update(['operator_id' => $operator->id]);
            return response()->json([
                'message' => 'Operator Updated successfully.',
                'title' => 'Leafing Updated.',
                'class' => 'bg-success'
            ]);
        }
        return response()->json([
    ...
    public function updateCounter(Request $request){
        ...
        try {
            ...
            DB::transaction(function () use ($request) {
                ...
                $leafing = Leafing::findOrFail($request->id);

                if (
                    empty($leafing->operator_id) ||
                    empty($leafing->admin_id)
                ) {
    ...
    public function cancel(Request $request){
        try {
            $leafing = Leafing::findOrFail($request->id);
            JobCardStageService::cancelStage($leafing->stage);
