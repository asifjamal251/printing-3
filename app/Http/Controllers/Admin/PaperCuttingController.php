...
use App\Http\Resources\Admin\PaperCutting\PaperCuttingCollection;
...
class PaperCuttingController extends Controller
{
    public function index(Request $request){
        if ($request->ajax()) {
            ...
            return response()->json(new PaperCuttingCollection($datas));
        }
        return view('admin.paper-cutting.list');
    }

    public function updateOperator(Request $request){
        if($request->operator_id){
            $cutting = PaperCutting::where('id', $request->id)->first();
            $operator = Operator::where('id', $request->operator_id)->first();
            $cutting->update(['operator_id' => $operator->id, 'admin_id' => $operator->admin_id]);
            JobCardStage::where('id', $cutting->job_card_stage_id)->update(['operator_id' => $operator->id]);
            return response()->json([
                'message' => 'Operator Updated successfully.',
                'title' => 'Paper Cutting Updated.',
                'class' => 'bg-success'
            ]);
        }
        return response()->json([
    ...
    public function updateCounter(Request $request){
        ...
        try {
            ...
            DB::transaction(function () use ($request) {
                ...
                $paperCutting = PaperCutting::findOrFail($request->id);

                if (
                    empty($paperCutting->operator_id) ||
                    empty($paperCutting->admin_id)
                ) {
    ...
    public function cancel(Request $request){
        try {
            $paper_cutting = PaperCutting::findOrFail($request->id);
            JobCardStageService::cancelStage($paper_cutting->stage);
