...
use App\Http\Resources\Admin\Coating\CoatingCollection;
...
class CoatingController extends Controller
{
    public function index(Request $request){
        if ($request->ajax()) {
            ...
            return response()->json(new CoatingCollection($datas));
        }
        return view('admin.coating.list');
    }

    public function updateOperator(Request $request){
        if($request->operator_id){
            $coating = Coating::where('id', $request->id)->first();
            $operator = Operator::where('id', $request->operator_id)->first();
            $coating->update(['operator_id' => $operator->id, 'admin_id' => $operator->admin_id]);
            JobCardStage::where('id', $coating->job_card_stage_id)->update(['operator_id' => $operator->id]);
    ...
    public function updateCounter(Request $request){
        ...
        try {
            ...
            DB::transaction(function () use ($request) {
                ...
                $coating = Coating::findOrFail($request->id);

                if (
                    empty($coating->operator_id) ||
                    empty($coating->admin_id)
                ) {
    ...
    public function cancel(Request $request){
        try {
            $coating = Coating::findOrFail($request->id);
            JobCardStageService::cancelStage($coating->stage);
