...
class SpotUVController extends Controller
{
    ...
    public function updateOperator(Request $request){
        if($request->operator_id){
            $spot_uv = SpotUV::where('id', $request->id)->first();
            $operator = Operator::where('id', $request->operator_id)->first();
            $spot_uv->update(['operator_id' => $operator->id, 'admin_id' => $operator->admin_id]);
            JobCardStage::where('id', $spot_uv->job_card_stage_id)->update(['operator_id' => $operator->id]);
            return response()->json([
                'message' => 'Operator Updated successfully.',
                'title' => 'Spot UV Updated.',
                'class' => 'bg-success'
            ]);
        }
        return response()->json([
    ...
    public function updateCounter(Request $request){
        ...
        try {
            ...
            DB::transaction(function () use ($request) {
                ...
                $spot_uv = SpotUV::findOrFail($request->id);

                if (
                    empty($spot_uv->operator_id) ||
                    empty($spot_uv->admin_id)
                ) {
    ...
    public function cancel(Request $request){
        try {
            $spot_uv = SpotUV::findOrFail($request->id);
            JobCardStageService::cancelStage($spot_uv->stage);
